// GitHub REST API helpers — OAuth token exchange + Contents API push.
...
/**
 * Push files to a GitHub repo using the Contents API (one PUT per file).
...
export async function pushFilesToRepo(
	...
	// Validate token and verify owner matches.
	...
	if (tokenOwner.toLowerCase() !== owner.toLowerCase()) {
		return { error: `Token milik @${tokenOwner} tapi repo owner adalah @${owner}. Buka Settings → GitHub → Disconnect, lalu Connect ulang.` };
	}

	// 1. Ensure repo exists — create with auto_init so it has a default branch.
	...
	if (!repoCheck.ok) {
		...
		// Wait for GitHub to finish initializing git storage.
		await new Promise((r) => setTimeout(r, 2000));
	}

	// 2. Push each file via Contents API.
	// GET first to obtain current file SHA (required when updating an existing file).
	let lastCommitSha = '';
	for (const file of files) {
		const existing = await githubRequest<{ sha: string }>(
			token, 'GET', `${repoPath}/contents/${file.path}`
		);
		const currentSha = existing.ok ? existing.data.sha : undefined;

		const result = await githubRequest<{ commit: { sha: string } }>(
			token, 'PUT', `${repoPath}/contents/${file.path}`,
			{
		...
		if (!result.ok) {
			console.error(`[push] PUT ${file.path} → ${(result as { ok: false; status: number }).status} ${result.message}`);
			return { error: `[put:${file.path}] ${result.message}` };
		}
		lastCommitSha = result.data.commit.sha;
	}

	return { repoUrl: `https://github.com/${owner}/${repoName}`, commitSha: lastCommitSha };
}
