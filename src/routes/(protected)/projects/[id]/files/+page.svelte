<script lang="ts">
	...
	function startPolling() {
		...
		pollTimer = setInterval(async () => {
			try {
				const res = await fetch(`/api/v1/projects/${data.project.id}`);
				if (!res.ok) return;
				const json = (await res.json()) as { data?: { cfPagesUrl?: string | null } };
				if (json.data?.cfPagesUrl) {
					cfPagesUrl = json.data.cfPagesUrl;
					building = false;
					stopPolling();
				}
	...
	// Resume polling if user navigated away mid-build.
	$effect(() => {
		if (syncedVersion !== null && !cfPagesUrl) {
			building = true;
			startPolling();
		}
		return stopPolling;
		...
		try {
			...
			if (res.ok) {
				const json = (await res.json()) as {
					data: { repoUrl: string; syncedVersion: number; commitSha: string; deploySecretsSet: boolean };
				};
				syncedVersion = json.data.syncedVersion;
				commitSha = json.data.commitSha;
				// After sync, GitHub Actions will build → upload dist → webhook sets cfPagesUrl.
				if (!cfPagesUrl) {
					building = true;
					startPolling();
				}
...
<!-- Deploy status banner — shown once GitHub sync has happened -->
{#if syncedVersion !== null && isViewingLatest}
	<div class="mb-4 flex flex-wrap items-center gap-2.5 rounded-xl border bg-card px-4 py-3 text-sm">
		{#if cfPagesUrl}
			<span class="size-2 shrink-0 animate-pulse rounded-full bg-green-500"></span>
			<span class="font-medium text-green-700 dark:text-green-400">Live</span>
			<a href={cfPagesUrl} target="_blank" rel="noopener noreferrer"
				class="min-w-0 truncate text-xs text-muted-foreground underline-offset-2 hover:underline">
				{cfPagesUrl}
			</a>
			<a href={cfPagesUrl} target="_blank" rel="noopener noreferrer"
				class="ml-auto shrink-0 rounded-lg border px-2.5 py-1 text-xs transition hover:bg-muted">
				Open ↗
			</a>
		{:else if building}
			<span class="size-3.5 shrink-0 animate-spin rounded-full border-2 border-primary border-t-transparent"></span>
			<span class="font-medium">Building & deploying…</span>
			<span class="text-xs text-muted-foreground">GitHub Actions sedang build (~2–3 menit)</span>
		{/if}
	</div>
{/if}
