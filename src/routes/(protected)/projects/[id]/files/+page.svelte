<script lang="ts">
	...
	let syncResult = $state<{ repoUrl: string } | { error: string } | null>(null);
	...
	async function pushToGithub() {
		...
		try {
			...
			if (res.ok) {
				const json = (await res.json()) as { data: { repoUrl: string } };
				syncResult = { repoUrl: json.data.repoUrl };
			} else {
				const json = (await res.json()) as { error?: { message?: string } };
				syncResult = { error: json.error?.message ?? 'Sync failed.' };
			}
		} catch {
...
{#if syncResult}
	{#if 'repoUrl' in syncResult}
		<div class="mb-4 rounded-md border border-success/40 bg-success/10 px-4 py-3 text-sm">
			Pushed successfully →
			<a
